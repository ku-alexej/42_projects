...
/*   Updated: 2023/06/13 10:45:11 by akurochk         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

static int	put_hex_lower(unsigned int nmbr, int zeroes)
{
	int	len;
	...
	while (zeroes-- > 0)
		len += ft_put_char('0');
	if (nmbr / 16)
		len += put_hex_lower((nmbr / 16), 0);
	len += ft_put_char(HEX_LOWER[nmbr % 16]);
	return (len);
}

static int	ft_x_lower(t_format *format, unsigned int u, int u_len, int zeroes)
{
	int	len;
...
int	ft_print_x_lower(t_format *format, va_list list)
{
	...
	u_len += zeroes;
	len += ft_x_lower(format, u, u_len, zeroes);
	return (len);
}
